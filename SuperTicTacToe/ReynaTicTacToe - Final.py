...
class SuperTicTacToeGame(QtWidgets.QMainWindow):
    ...
    def launch_winner_ui(self, winner):
        ...
        UIUtils.remove_window_header(self.winner_window)
        UIUtils.apply_rounded_corners(self.winner_window)
        UIUtils.setup_window_drag(self.winner_window)
    ...
    def launch_draw_ui(self):
        CLICK_SOUND.play()
        self.draw_window = QtWidgets.QMainWindow()
        uic.loadUi('.ui/playerdraw.ui', self.draw_window)
        UIUtils.remove_window_header(self.draw_window)
        UIUtils.apply_rounded_corners(self.draw_window)
        UIUtils.setup_window_drag(self.draw_window)
        wins_label = self.draw_window.findChild(QtWidgets.QLabel, 'wins') 
        ...
        if wins_label:
            ...
            print("Warning: QLabel 'wins' not found. Check UI for correct naming.")

        rematch_button = self.draw_window.findChild(QtWidgets.QPushButton, 'rematch')
        back_button = self.draw_window.findChild(QtWidgets.QPushButton, 'back')
        rematch_button.clicked.connect(self.rematch_game)
        back_button.clicked.connect(self.back_to_main)
...
class SuperTicTacToe_AI(QtWidgets.QMainWindow):
    ...
    def is_draw(self):
        all_full_or_won = all(
            self.super_board[i][j].is_full() or self.super_board[i][j].winner is not None
            for i in range(3) for j in range(3)
        )
        return all_full_or_won and not self.main_winner
